...
def clean_and_convert(value, type):
    value = value.replace('â€¯', '').replace('â€', '').replace('”', '0').replace('EUR', '').replace('âˆ’', '-').replace('%', '').strip()
...
with open('ausgewaehlte_spaltenda.csv', 'r') as csv_file:
...
file_paths = [
    "C:/Users/fawad/AppData/Roaming/MetaQuotes/Terminal/81A933A9AFC5DE3C23B15CAB19C63850/MQL5/Files/TVdax.txt",
